...
class SudokuDetailProcessorSuite extends munit.FunSuite with SudokuTestHelpers:
  ...
  test(
    "Sending an update to a fresh instance of the SudokuDetailProcessor that sets one cell to a single value should result in sending an update that reflects this update"):
      val probe = testKit.createTestProbe[SudokuDetailProcessor.Response]()
      val detailProcessor = testKit.spawn(SudokuDetailProcessor[Row](id = 0))
      detailProcessor ! Update(Vector((4, Set(7))), probe.ref)

      val expectedState1 =
        SudokuDetailProcessor.RowUpdate(
          0,
          stringToIndexedUpdate(
            Vector(
              "123456 89",
              "123456 89",
              "123456 89",
  ...
  test(
    "Sending a series of subsequent Updates to a SudokuDetailProcessor should result in sending updates and ultimately return no changes"):
        ...
        Update(
          stringToReductionSet(
            Vector(
              ...
              "      78 ", // (7,8): Isolated & complete
              "     6789",
              " 23   78 ")).zipWithIndex.map { _.swap },
          detailParentProbe.ref)

      detailProcessor ! update1

      val reducedUpdate1 = BlockUpdate(
        2,
        stringToReductionSet(
          Vector(
            " 23 56   ",
      ...
      val reducedUpdate2 =
        BlockUpdate(2, stringToIndexedUpdate(Vector("", "", "", "", "", "", "", "     6   ", "")))
      ...
      val reducedUpdate3 =
        BlockUpdate(2, stringToIndexedUpdate(Vector(" 23 5    ", "", "", " 2  5    ", "", "", "", "", "")))
