...
class ReductionRuleSuite extends munit.FunSuite with SudokuTestHelpers:
  test(
    "Applying reduction rules should eliminate values in isolated complete sets from occurrences in other cells (First reduction rule)"):
      ...
      /** Note: test data in this test is a "ReductionSet": is consists of 9 cells (e.g. in a row or column or 3x3
        * block). Each cell is represented by 9 characters: a space encodes a number not present in the cell, characters
        * '1' through '9' encode the presence of that digit in the cell.
        */
